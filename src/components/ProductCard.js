...
export default function ProductCard({ image, title, description, alt }) {
  const { t } = useLanguage();
  const waMsg = encodeURIComponent(`नमस्ते, मुझे ${title} के बारे में जानकारी और कीमत चाहिए।`);
  ...
  return (
    <div className="group rounded-xl overflow-hidden bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 hover:shadow-xl transition-all hover:-translate-y-1">
      ...
      <div className="p-4">
        ...
        <p className="text-base text-gray-600 dark:text-gray-400 line-clamp-2 mb-3">{description}</p>
        <div className="flex items-center gap-3">
          <a
            href="tel:8299200015"
            className="inline-flex items-center gap-1.5 text-base font-semibold text-primary hover:text-primary-dark transition-colors"
          >
